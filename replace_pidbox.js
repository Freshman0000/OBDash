...
const oldPidBoxRegex = /const PidBox: React\.FC<\{ label: string; value: string \| number; unit: string; color: string; align\?: 'left' \| 'right'; borderSide\?: 'left' \| 'right' \}> = \(\{ label, value, unit, color, align = 'left', borderSide = 'left' \}\) => \([\s\S]*?\}\s*\/>\s*<\/div>\s*<\/div>\s*\);/m;
